...
bench of Section~\ref{sec:hil} --- is recorded in
Table~\ref{tab:lqry-gains} and registered in the
repository~\cite{repo}. Two structural remarks: the pitch design
carries the actuator as a fifth synthesis state (the 24\,rad/s lag),
so its gain vector feeds back the elevator alongside the rigid-body
states; and the $2\times2$ lateral design provides for a
sideslip-integral gain alongside the bank integral, but in this
delivery the $\beta$-integral column is numerically zero --- only the
bank integral is active.

\begin{table}[!t]
\caption{Gains of the fixed-gain LQRy delivery (4 significant
digits). Convention: $\Delta u=G_{x}\,(x-x^{e})+K_{I}\!\int\!e\,dt$,
the increments adding to the trim commands of~\eqref{eq:dtrim}. The
$K_I$ entries of PhiHold are the bank ($\phi$) integral gains.}
...
$[-0.004694\;5.362\;-10.69\;-95.34\;-5.477]$ & $-25$ \\
AltitudeHold & $[\VT\,\alpha\,q\,\theta\,h]$ &
$[-0.07210\;4.535\;-0.2142\;-5.140\;-0.4439]$ & $-0.04$ \\
VelHold & $[\VT\,\alpha\,q\,\theta]$ &
$[-0.7506\;-0.5861\;0.006315\;0.6642]$ & $-0.4410$ \\
PhiHold (ail.) & $[\beta\,p\,r\,\phi]$ &
$[0.01697\;0.004305\;0.01497\;0.06072]$ & $0.003883$ \\
PhiHold (rud.) & $[\beta\,p\,r\,\phi]$ &
$[-0.003758\;-0.0009542\;-0.003318\;-0.01346]$ & $-0.0008727$ \\
PsiHold & $[\beta\,p\,r\,\phi\,\psi]$ &
$[-9.147\;-0.1859\;0.1492\;-1.994\;-0.6554]$ & $-0.03366$ \\
\bottomrule
\end{tabular}}
\end{table}

Unlike the PID harness, the LQRy delivery harness contains no
actuator models at all: the commanded increments, added to the trim
commands of~\eqref{eq:dtrim}, drive the plant inputs directly --- no
lags, no rate limits, no position or throttle saturations, no
reference prefilter, and no $\theta^{ref}$ clamp --- and references
enter the loops as raw steps. The 24\,rad/s lag lives in the
\emph{synthesis} models only; on the rig, the fifth gain of the pitch
loop multiplies the commanded elevator itself. (Every statement in
this paragraph was verified block by block on the archived delivery
rig.)
