...
fixed-wing longitudinal dynamics~\cite{ingabire2019control}. Three
gaps recur in this literature. First, most comparisons target
rotary-wing platforms; the fixed-wing mode of a hybrid vehicle is
rarely the object of study. Second, the comparisons are usually
confined to simulation of isolated channels, rather than a full
mission exercising the longitudinal and lateral--directional loops
together on the embedded implementation. Third, robustness ---
arguably where the two design philosophies should differ most --- is
seldom part of the protocol, even though LQ designs carry classical
margin guarantees~\cite{safonov1977gain} that do not survive output
...
The present work extends that lineage in three directions: the plant
...
\begin{itemize}
  \item the design of a cascade PID architecture for the fixed-wing
        ...
        gain-scheduled output-feedback LQR (LQRy) designed for the same
        platform, both judged against a common set of
        literature-grounded closed-loop requirements;
  \item a robustness study showing that a severe, sign-changing
        lateral--directional model mismatch defeats \emph{both}
        compared laws --- the aggressively tuned LQRy diverging
        outright, while an earlier, softer fixed-gain iteration of the
        same structure survives it, isolating tuning (not structure)
        as what buys this robustness --- and
        exposing \emph{complementary} weaknesses in the remaining
        scenarios: under a longitudinal-only mismatch the LQRy stays
        nominal while the PID's clamped altitude loop drifts, and
        under wind and gusts the LQRy concedes altitude where the PID
        stays firm;
