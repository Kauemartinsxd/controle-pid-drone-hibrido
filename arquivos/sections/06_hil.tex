...
is the auto-generated production code. It covers the controller
discretization and code generation, the bench architecture and its
link-level validation, the bit-determinism of the lockstep loop and
the sensitivity study that determinism enables, and the embedded
campaign results --- closing with the controller comparison repeated
on the hardware.
...
\subsubsection{LQRy} The embedded LQRy is exactly the fixed-gain law
of Section~\ref{sec:lqry} compared throughout this article, with the
gain set registered in the repository~\cite{repo} --- the bench
therefore closes the comparison on the same two builds evaluated in
SIL. The LQRy campaign was executed under the same
protocol, with per-case firmware builds and scripted programming of the
target. Its SIL--HIL residual is larger than the PID's --- from $0.23^\circ$ to
...
to drive the conclusions. Table~\ref{tab:hil-comparison} reports the
combined mission executed on the hardware: the bench reproduces the
SIL verdict of Section~\ref{sec:sil-comparison} essentially unchanged
--- the PID tracks airspeed and heading substantially more tightly
(RMS 0.07\,m/s and $1.27^\circ$ against 0.37\,m/s and $2.92^\circ$)
with altitude comparable (1.63 versus 1.78\,m), and the LQRy's single
requirement violation remains the airspeed overshoot ($14.6\%$ on the
bench against $16.5\%$ in SIL, the small difference owing to the
start-up term above). The maxima occur at the reference-step
transients. Fig.~\ref{fig:justo} shows flight case~1.

\begin{table}[!t]
\caption{Embedded comparison on the bench (HIL, combined mission,
identical references, $t>12$\,s).}
\label{tab:hil-comparison}
\centering
\begin{tabular}{lcccc}
\toprule
...
\midrule
$\VT$ [m/s]    & 0.07 & 0.76 & 0.37 & 2.11 \\
$\psi$ [$^\circ$] & 1.27 & 5.79 & 2.92 & 10.16 \\
$h$ [m]        & 1.63 & 6.82 & 1.78 & 10.00 \\
\bottomrule
\end{tabular}
\end{table}
