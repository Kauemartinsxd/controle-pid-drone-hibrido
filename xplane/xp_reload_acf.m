...
fprintf(fid, '%s\n', ...
    ...
    '# (2026-09-08) ativa PELO HANDLE do processo X-Plane: a janela do Plane Maker', ...
    '# tambem se chama X-System e AppActivate(titulo) — e ate AppActivate(pid) —', ...
    '# deixavam o Plane Maker na frente (cliques/ESC iam para ele). SendKeys("%")', ...
    '# (ALT) libera o SetForegroundWindow a partir de um processo em 2o plano.', ...
...
global XP_ACF_SIG
if isempty(XP_ACF_SIG)   % default = assinaturas CONHECIDAS (struct array); XP_ACF_SIG = 'off' desliga
    % (2026-09-08) v1.3-lat = v1.2 com raios yaw 1,12 / roll 0,84 ft (inercias
    % da Ana: Izz 0,257 / Ixx 0,144 kg.m2) + aileron crat 0,45 — a aplicar no
    % Plane Maker pelo Kaue (VALIDACAO_VOO_REAL.md, secao 09-08). Qualquer
    % uma das duas passa; o nome impresso diz qual carregou.
    XP_ACF_SIG = [struct('J', [0.2008 0.2008 0.0786], 'm', 2.223, 'elev', 25, 'nome', 'gemeo v1.2 (md5 5b808d0b)'), ...
                  struct('J', [0.2008 0.1157 0.0648], 'm', 2.223, 'elev', 25, 'nome', 'gemeo v1.3-lat (inercias da Ana + aileron 0.45)'), ...
                  struct('J', [0.0520 0.1157 0.0648], 'm', 2.223, 'elev', 25, 'nome', 'gemeo v2 candidato, cursos 25 (v1.3-lat + Iyy da Ana, CG +7 cm, profundor 0.5, h-stab x1.20)'), ...
                  struct('J', [0.0520 0.1157 0.0648], 'm', 2.223, 'elev', 15, 'nome', 'gemeo v2 (2026-09-08: inercias da Ana, CG 0.20 ft, h-stab 0.37 ft/-0.2 deg, profundor 0.20, aileron 0.25 em 2 elementos, cursos 15)')];
end
if isstruct(XP_ACF_SIG)
    ...
    i_ok = 0;
    for k_ = 1:numel(XP_ACF_SIG)
        okJ = all(abs(sg(1:3) - XP_ACF_SIG(k_).J) < 0.01); okm = abs(sg(4) - XP_ACF_SIG(k_).m) < 0.05; oke = abs(sg(5) - XP_ACF_SIG(k_).elev) < 0.5;
        if okJ && okm && oke, i_ok = k_; break; end
    end
    if i_ok == 0
        esp = strjoin(arrayfun(@(a) sprintf('%s J=[%.4f %.4f %.4f] m=%.3f elev=%.0f', a.nome, a.J, a.m, a.elev), XP_ACF_SIG, 'UniformOutput', false), ' | ');
        error(['xp_reload_acf: AERONAVE ERRADA carregada! lido J=[%.4f %.4f %.4f] m=%.3f elev=%.0f; ' ...
               'esperado: %s. O clique abriu outra linha do dialogo: ' ...
               'iguale TODOS os DH-Lon-REV-03*.acf da pasta ao ativo (XP_sync_acf_clones).'], ...
               sg(1), sg(2), sg(3), sg(4), sg(5), esp);
    end
    fprintf('xp_reload_acf: assinatura OK = %s (J %.3f/%.3f/%.3f, m %.2f kg, elev %.0f deg).\n', XP_ACF_SIG(i_ok).nome, sg(1), sg(2), sg(3), sg(4), sg(5));
end
end
